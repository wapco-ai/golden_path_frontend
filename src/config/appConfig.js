...
const defaultDestinationsBaseUrl = `${defaultApiBaseUrl}/api/v1/destinations`;
const defaultDestinationSuggestionsUrl = `${defaultDestinationsBaseUrl}/suggestions`;
...
const appConfig = {
  ...
  destinationsBaseUrl: (import.meta?.env?.VITE_DESTINATIONS_BASE_URL?.trim() || defaultDestinationsBaseUrl).replace(/\/$/, ''),
  destinationSuggestionsUrl: (import.meta?.env?.VITE_DESTINATION_SUGGESTIONS_URL?.trim() || defaultDestinationSuggestionsUrl).replace(/\/$/, ''),
