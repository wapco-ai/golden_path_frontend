...
const Amain = () => {
  ...
  const setSubMenuState = useCallback((menuIds, isOpen) => {
    setOpenSubMenus((prev) => {
      const next = new Set(prev);
  ...
  const isSubMenuOpen = useCallback((menuId) => openSubMenus.has(menuId), [openSubMenus]);
  useEffect(() => {
    if ((isVanEdgesLayerActive || isVanNodesLayerActive) && activeMenu === 'mapmanage' && !isSubMenuOpen(3)) {
      setSubMenuState([3], true);
    }
  ...
  }, [activeMenu, isSubMenuOpen, isVanDrawingMode, isVanEdgesLayerActive, isVanNodesLayerActive, setSubMenuState]);
  useEffect(() => {
  ...
  }, [isTempAreaLayerActive, isTempAreaDrawingMode]);
  useEffect(() => {
    const subMenusToSync = [0, 1, 2, 3];

    if (activeMenu === 'mapmanage' && activeEditableLayer) {
      setSubMenuState(subMenusToSync, true);
    } else {
      setSubMenuState(subMenusToSync, false);
    }
  }, [activeEditableLayer, activeMenu, setSubMenuState]);
              ...
              <div className="map-container">
                ...
                <div className="map-control-top-left">
                  ...
                  <div className="action-buttons-group">
                    ...
                    {/* Button 1 */}
                    <div className={`action-button temp-area-manage ${isSubMenuOpen(1) ? 'selected' : ''}`}
                      onClick={() => {
                        toggleSubMenu(1);
                    ...
                    {/* Button 2 */}
                    <div className={`action-button manage-area ${isSubMenuOpen(2) ? 'selected' : ''}`}
                      onClick={() => {
                        toggleSubMenu(2);
                        // Reset location marker mode when other buttons are clicked
                        setIsLocationMarkerMode(false);
                    ...
                    {/* Button 3 */}
                    <div className={`action-button van-manage ${isSubMenuOpen(3) ? 'selected' : ''}`}
                      onClick={() => {
                        toggleSubMenu(3);
                        // Reset location marker mode when other buttons are clicked
                        setIsLocationMarkerMode(false);
