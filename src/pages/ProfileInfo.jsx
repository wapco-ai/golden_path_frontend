...
const daysInCalendarMonth = (year, month, calendarPartsFn, findGregorianDateFn) => {
  if (!year || !month) return [];

  const startMatch = findGregorianDateFn(year, month, 1);
  if (!startMatch) return [];

  const days = [];
  let cursor = new Date(startMatch);
  while (true) {
    const parts = calendarPartsFn(cursor);
...
function ProfileInfo() {
  ...
  const calendarParts = useCallback((date) => {
  ...
  const findGregorianDateByCalendarParts = useCallback((targetYear, targetMonth, targetDay = 1) => {
  ...
  const monthNames = useMemo(() => {
    ...
    return names;
  }, [activeCalendar.calendar, activeCalendar.locale, calendarFormatter, calendarParts, findGregorianDateByCalendarParts]);
  ...
  const dayOptions = useMemo(
    ...
    [birthDateParts.month, birthDateParts.year, calendarParts, findGregorianDateByCalendarParts]
  );
