  ...
  const handleFeedbackSubmit = async () => {
    ...
    try {
      ...
      if (status === 400 || status === 422) {
        toast.error(intl.formatMessage({ id: 'supportFeedbackInvalid' }));
      } else if (!error?.response) {
        toast.error(intl.formatMessage({ id: 'supportFeedbackNetworkError' }));
      } else {
        toast.error(intl.formatMessage({ id: 'supportFeedbackSubmitError' }));
      }
    } finally {
