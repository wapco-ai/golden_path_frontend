...
export class HybridNavigationEngine {
  ...
  checkGpsTimeout(now = Date.now()) {
    if (!this.canUseDr()) return;
    if (!this.lastGpsAt) return;
    if (now - this.lastGpsAt > this.config.gpsNoUpdateTimeoutMs) {
      this.log('gps_timeout_to_dr', { elapsedMs: now - this.lastGpsAt });
      this.transition(NAV_STATES.DR_MODE);
    }
  }

  handleGpsUnavailable(reason = 'gps_error') {
    this.log('gps_unavailable', { reason });
    if (this.canUseDr()) {
      this.transition(NAV_STATES.DR_MODE);
    }
  }
  ...
  async handleGps(gps) {
    ...
    const badForLong = badAccuracy && this.lastGoodGpsAt > 0 && now - this.lastGoodGpsAt > this.config.gpsBadDurationMs;

    if ((badForLong || jump) && this.canUseDr()) {
      this.transition(NAV_STATES.DR_MODE);
